...
func TestUnionEnumDiscriminantRoundTrip(t *testing.T) {
	in := testUnionEnumDiscriminant{Tag: 1, Value1: 0xdeadbeef}
	b, err := Marshal(&in)
	assert.NoError(t, err)
	assert.Equal(t, "01000100efbeadde", hex.EncodeToString(b[20:28]))

	var out testUnionEnumDiscriminant
	assert.NoError(t, NewDecoder(bytes.NewReader(b)).Decode(&out))
...
type testUnionEnumDiscriminant struct {
	Tag    uint64 `ndr:"unionTag,enum"`
